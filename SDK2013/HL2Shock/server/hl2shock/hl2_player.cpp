...
BEGIN_DATADESC( CHL2_Player )
	...
	//SHOCK
	DEFINE_THINKFUNC(HealShockPlayer),
	...
	*/
	...
	// SHOCK
	DEFINE_FIELD (m_intHealthHypoStock, FIELD_INTEGER),
...
IMPLEMENT_SERVERCLASS_ST(CHL2_Player, DT_HL2_Player)
	...
	SendPropInt(SENDINFO(m_intHealthHypoStock)),
END_SEND_TABLE()
...
void CHL2_Player::Spawn(void)
{
	...
	// SHOCK
	m_intHealthHealed = 0;
	RegisterThinkContext("HealthHypoThink");
	SetContextThink(&CHL2_Player::HealShockPlayer, gpGlobals->curtime, "HealthHypoThink");
}
...
{
	// Don't do nothing now, ignore weapon_stunsticks!
#ifndef HL2MP	
...
// SHOCK STUFF STARTS HERE!!!
void CHL2_Player::HealPlayerByHypo()
{
	if (m_intHealthHypoStock <= 0)
	{
		DevMsg("CHL2_Player: No MedHypos in stock!");
		return;
	}

	if (GetHealth() >= GetMaxHealth())
	{
		DevMsg("CHL2_Player: We are at max health. Not healing!");
		return;
	}
		
	m_intHealthHypoStock = m_intHealthHypoStock - 1;

	CPASAttenuationFilter filter(this);
	filter.UsePredictionRules();
	EmitSound(filter, entindex(), "HealthKit.Touch");

	SetNextThink(gpGlobals->curtime, "HealthHypoThink");
}

void CHL2_Player::HealShockPlayer()
{
	if (m_intHealthHealed < 8)
	{ 
		TakeHealth(1, DMG_GENERIC);
		m_intHealthHealed++;
		SetNextThink(gpGlobals->curtime + 1.25, "HealthHypoThink");
	}
	else if (m_intHealthHealed >= 8)
	{
		m_intHealthHealed = 0;
	}
}

void CC_HealWithHypo(void)
{
	...
	pPlayer->HealPlayerByHypo();
}
static ConCommand shock_heal_player("shock_heal_player", CC_HealWithHypo, "Heal the player. \n", FCVAR_CHEAT);
