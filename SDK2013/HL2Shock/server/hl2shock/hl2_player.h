...
public:
	...
	// We need a special function to slowly heal the player after taking a hypo.
	void HealShockPlayer();
	void HealPlayerByHypo();
	void GiveMedHypo()
	{
		m_intHealthHypoStock++;
	}

	int GetMedHypoAmmount()
	{
		return m_intHealthHypoStock;
	}
...
private:
	...
	// Shock
	CNetworkVar(int, m_intHealthHypoStock);	// number of shots fired recently
	int m_intHealthHealed;
...
#ifdef _DEBUG
	Assert(dynamic_cast<CHL2_Player*>(pEntity) != 0);
#endif
